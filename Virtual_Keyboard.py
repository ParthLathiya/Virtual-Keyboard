...
class Button:
    def __init__(self, pos, text, size=(85, 85)):
        ...
        self.size = size
        self.text = text
        self.last_pressed_time = 0  # Track the last time the button was pressed
...
def draw_buttons(img, button_list):
    ...
    Returns:
        np.ndarray: The image with the buttons drawn.
    """
    overlay = img.copy()
...
for k in range(len(keyboard_keys)):
    for x, key in enumerate(keyboard_keys[k]):
        if key not in ["SPACE", "ENTER", "BACKSPACE"]:
            ...
            button_list.append(Button((100 * x + 780, 100 * k + 50), key, (400, 85)))
        elif key == "BACKSPACE":
            button_list.append(Button((100 * x + 140, 100 * k + 50), key, (400, 85)))

cooldown_time = 0.5  # Cooldown time of 0.5 seconds to prevent spamming
